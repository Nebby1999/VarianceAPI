...
public class VariantComponentTreeView : TreeListControl
{
    ...
    public VariantComponentTreeView()
    {
    }
    ...
    public void SetRootItem(string Header)
    {
        ...
        RootItem.DataContext = new VariantComponentTreeInfo();
    ...
    public void PopulateItem(Type type)
    {
        ...
        if (attachPoint == null)
        {
            attachPoint = RootItem.AddItem(assemblyName, false, true, new VariantComponentTreeInfo(assemblyName));
        ...
        foreach (var ns in namespaces)
        {
            ...
            if (next == null)
            {
                attachPoint = attachPoint.AddItem(ns, false, false, new VariantComponentTreeInfo(ns));
                AddHandlerEvents(attachPoint);
            }
            else
                attachPoint = next;
        }
        var state = attachPoint.AddItem(type.Name, true, false, new VariantComponentTreeInfo(type));
    ...
    private void PrepareDragDrop(object sender, System.EventArgs args)
    {
        ...
        try
        {
            ...
            var treeInfo = (VariantComponentTreeInfo)item.DataContext;
            if (treeInfo.itemType != ItemType.variantComponent)
                return;
    ...
    public override void AssignDefaults()
    {
        ...
        VariantComponentDrawerLocator locator = ScriptableObject.CreateInstance<VariantComponentDrawerLocator>();
        MonoScript script = MonoScript.FromScriptableObject(locator);
        locator.hideFlags = HideFlags.HideAndDontSave;
        var componentDrawerPath = Path.GetDirectoryName(AssetDatabase.GetAssetPath(script)) + "/Gizmos/";
        textureVariantComponentIcon = GetTexture(componentDrawerPath + "entity_state.png");
        textureFolderIcon = GetTexture(componentDrawerPath + "folder.png");
    ...
    public Texture2D textureVariantComponentIcon;
    ...
    public void CustomIconHandler(object sender, System.EventArgs args)
    {
        ...
        if (ItemType.variantComponent == treeInfo.itemType)
            ShowButtonTextureInternal(textureVariantComponentIcon);
        else
            ShowButtonTextureInternal(textureFolderIcon);
    }


    public class VariantComponentTreeInfo
    {
        public ItemType itemType;
        public string shortHandName;
        public string fullName;
        public VariantComponentTreeInfo(Type variantComponent)
        {
            shortHandName = variantComponent.Name;
            fullName = variantComponent.AssemblyQualifiedName;
            itemType = ItemType.variantComponent;
        }
        public VariantComponentTreeInfo(string label)
        {
            shortHandName = label;
            itemType = ItemType.Namespace;
        }

        public VariantComponentTreeInfo()
        {
