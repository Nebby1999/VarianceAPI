...
namespace VarianceAPI
{
    public class VariantHandler : NetworkBehaviour
    {
        ...
        private void Awake()
        {
            ...
            {
                if (RunArtifactManager.instance.IsArtifactEnabled(ContentPacks.serializableContentPack.artifactDefs[0]))
                {
        ...
        public void Modify()
        {
            if (this.unique)
            {
                VAPILog.LogI(identifierName + " is unique, attempting to remove unecesary componenets.");
            ...
            if (this.body)
            {
                if (this.tier >= VariantTier.Rare && this.arrivalMessage != "")
                {
                    ...
                    {
                        VAPILog.LogI(identifierName + " Variant is Rare or Legendary but doesnt have an arrival message set! using generic message.");
        ...
        private void ModifyStats()
        {
            ...
            foreach(float multiplier in multipliers)
            {
                if(multiplier < 0)
                {
                    VAPILog.LogI("One of " + this.identifierName + "'s stat multipliers is set to a negative number! Continuing but expect unstable things, here be dragons!");
                }
            }
        ...
        private void AddItems()
        {
            if (this.master.inventory)
            {
                if (this.inventory != null)
                {
                        ...
                        for (int i = 0; i < inventory.counts.Length; i++)
                        {
                            ...
                            if (giveItem)
                            {
                                ...
                                else
                                {
                                    if(itemDef == null)
                                    {
                                        VAPILog.LogW("Could not find itemDef matching ItemString \"" + inventory.itemStrings[i] + "\". Aborting adding said item.");
                                    }
                                    if(amount <= 0)
                                    {
                                        VAPILog.LogW(inventory.itemStrings[i] + "'s amount was 0 or lower.");
                                    }
                                }   
                            }
                        }
                    ...
                    {
                        VAPILog.LogE(identifierName + "'s VariantInventory's arrays are not the same size! aborting adding items from the inventory!");
                    }
                }
        ...
        private void AddEquipment()
        {
            if(this.master.inventory)
            {
                if(customEquipment)
                {
                    ...
                    {
                        VAPILog.LogW("Could not find EquipmentDef matching EquipmentString \"" + customEquipment.equipmentString + "\". Aborting adding said Equipment.");
                    }
                }
            }
        }
                ...
                for (int i = 0; i < skillReplacements.Length; i++)
                {
                    switch (skillReplacements[i].skillSlot)
                    {
                        ...
                        case SkillSlot.Special:
                            ...
                            break;
                        case SkillSlot.None:
                            VAPILog.LogW(identifierName + "'s " + i + " skill replacement has its skill slot set to none.");
                            break;
                    }
                }
            }
                            ...
                            if (assemblies != null)
                            {
                                foreach (var assembly in assemblies)
                                {
                                    ...
                                    if (type != null)
                                    {
                                        ...
                                        {
                                            VAPILog.LogI("Adding " + type.Name + " Component to " + body.GetDisplayName());
            ...
            if(this.body)
            {
                if(this.buff != null)
                {
                    for (int i = 0; i < buff.Length; i++)
                    {
                        ...
                        {
                            VAPILog.LogW("Could not find BuffDef matching BuffString \"" + currentBuffIndex.buffDef + "\". Aborting adding said Buff.");
                        }
                    }
                }
            }
            ...
            foreach (VariantMeshReplacement meshReplacement in meshReplacements)
            {
                switch(meshReplacement.meshType)
                {
                    ...
                    case MeshType.BeetleGuard:
                        VAPILog.LogW("Beetle Guard MeshSwaps are not supported! your variant will not appear correctly ingame.");
                        RestoreEquipment();
                        break;
                    ...
                    case MeshType.MagmaWorm:
                        VAPILog.LogW("MagmaWorm MeshSwaps are not supported! your variant will not appear correctly ingame.");
                        RestoreEquipment();
                        break;
                    case MeshType.OverloadingWorm:
                        VAPILog.LogW("Overloading Worm MeshSwaps are not supported! your variant will not appear correctly ingame.");
                        RestoreEquipment();
                        break;
                }
            }
